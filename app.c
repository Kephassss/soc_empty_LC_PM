...
void sl_bt_on_event(sl_bt_msg_t *evt)
{
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_bt_evt_system_boot_id:
      // Initialise et démarre la diffusion BLE (Advertising) pour rendre le périphérique visible
    ...
    case sl_bt_evt_connection_opened_id:
      // Stocke l'identifiant de connexion dès qu'un client (smartphone) se connecte
      identifiant_connexion_active = evt->data.evt_connection_opened.connection;
      break;

    case sl_bt_evt_connection_closed_id:
      // Arrête le timer de mesure et réinitialise l'état de connexion lors de la déconnexion
      sl_sleeptimer_stop_timer(&gestionnaire_timer_temperature);
      identifiant_connexion_active = 0xff;
      break;

    case sl_bt_evt_gatt_server_user_read_request_id:
      // Répond à une demande de lecture ponctuelle du client sur la caractéristique température
    ...
    case sl_bt_evt_gatt_server_characteristic_status_id:
      // Gère l'activation ou la désactivation des notifications (via CCCD) par le client
    ...
    case sl_bt_evt_system_external_signal_id:
      // Envoie la valeur de température mise à jour au client connecté via une notification
    ...
    case sl_bt_evt_gatt_server_user_write_request_id:
      // Traite les commandes d'écriture du client (LEDs) et répond uniquement si un acquittement est requis
